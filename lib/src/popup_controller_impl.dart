...
class PopupControllerImpl implements PopupController {
  ...
  PopupControllerImpl({
    List<MarkerData> initiallySelectedMarkers = const <MarkerData>[],
  }) : selectedMarkersWithKeys = LinkedHashSet<MarkerWithKey>.from(
         initiallySelectedMarkers.map(
           (MarkerData marker) => MarkerWithKey(marker),
         ),
       );

  @override
  List<MarkerData> get selectedMarkers => selectedMarkersWithKeys
      .map((MarkerWithKey markerWithKey) => markerWithKey.marker)
      .toList();
